...
const Busqueda = () => {
    ...
    const {handleBuscador,buscador,projects} = useProject()

    const proyectosFiltrados = busqueda === '' ? [] : projects.filter(project => project.nombre.toLowerCase().includes(busqueda.toLowerCase()))
                    ...
                    {proyectosFiltrados.length > 0 && (
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {proyectosFiltrados.map(project => (
                                <Combobox.Option key={project._id}
                                         value={project}
